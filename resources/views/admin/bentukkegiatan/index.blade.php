            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table data-table" id="bentukkegiatan-datatable">
                            <thead>
                                <tr>
                                    <th>Title</th>
                                    <th>Description</th>
    ...
    <div class="modal fade text-left" id="modal-create" tabindex="-1" role="dialog" aria-labelledby="myModalLabel1"
        ...
        <div class="modal-dialog modal-dialog-scrollable" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('admin.bentukkegiatan.store') }}" method="POST" id="form-create">
                    @csrf
...
@push('scripts')
    ...
    <script>
            ...
            const columns = [{
                ...
                },
                {
                    render: function(data, type, full, row) {
                        return `
                    <button onclick="editBentukKegiatan(${full.id})" class="btn bg-warning text-light">Edit</button>
                    <button onclick="deleteBentukKegiatan(${full.id})" class="btn bg-danger text-light">Delete</button>
                    `;
                    }
                },
            ];

            $('#bentukkegiatan-datatable').DataTable({
                scrollCollapse: true,
                destroy: true,
                autoWidth: false,
                responsive: true,
                ...
                bInfo: true,
                ajax: {
                    url: "{{ route('admin.bentukkegiatan.index') }}",
                    // data: filterData
                },
                columns: columns,
                "lengthMenu": [
        ...
        function editBentukKegiatan(id) {
            var baseUrl = "{{ url('/') }}";
            var editUrl = baseUrl + `/admin/bentukkegiatan/${id}/edit`;
        ...
        function deleteBentukKegiatan(id) {
            var baseUrl = "{{ url('/') }}";
            var deleteUrl = baseUrl + `/admin/bentukkegiatan/${id}`;
